...
module Ai
  class SeoMetaGenerator < BaseGenerator
    ...
    def build_prompt(fields)
      ...
      <<~PROMPT
        あなたはSEOとSNSマーケティングの専門家です。
        以下の技術記事を分析し、検索エンジンとSNSに最適化されたメタデータを生成してください。
        ...
        **メタディスクリプション（検索結果表示用）**
        - 120〜155文字が最適（最大160文字厳守）
        - 記事の主要な価値提案を含める
        - 検索意図に応える内容にする
        - 自然な文章で、キーワードを無理に詰め込まない
        - 行動を促す表現を含める（「解説します」「学べます」など）
        ...
        **OGディスクリプション（SNS表示用）**
        ...
        以下のJSON形式で出力してください：
        {
          "meta_description": "SEO最適化された説明文",
          "meta_keywords": "キーワード1, キーワード2, キーワード3",
          "og_title": "SNS向けの魅力的なタイトル",
          "og_description": "SNSでシェアされやすい説明文"
        }
