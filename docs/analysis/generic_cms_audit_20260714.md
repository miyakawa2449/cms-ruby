...
### S1-6: パスキー（WebAuthn）導入【2026-07-16承認・新設】

秘密URL方式（難読化）からパスキー認証（攻撃自体の無効化）への転換。維持コスト削減とセキュリティ・UX向上を両立する。

- **Phase A**: パスワード+2FAと並行してパスキーログインを追加（`webauthn-ruby` + Deviseカスタムストラテジー）。複数デバイス登録を必須とするリカバリー設計が核心
- **Phase B**: 安定確認後、管理URLローテーション機構を廃止・簡素化（Resolver/Updater/RotationJob/履歴/通知メール/RouteReloaderの縮小 = 約7コンポーネント削減）。パスのENV設定自体は残す
- 進め方: 仕様書を作成 → 剛さんの承認 → TDD実装（認証系のためTDD必須領域)
- 備考: ステージ2のマルチ管理者と相性が良い（管理者ごとのデバイス登録）

### S1-8: 管理画面モバイル対応【2026-07-16承認・新設】

**方針（剛さん承認済み）: 「簡易モード+全モード」の2本立ては不採用**。1つのレスポンシブUI + タスク優先度分けで対応する（複製UIは必ず乖離する、というS1-5の教訓に基づく）。

- **優先度高（しっかりレスポンシブ化）**: 問い合わせ確認・状態変更、記事の公開/非公開切替、ダッシュボード、パスキー/2FA管理
- **優先度中（崩れない程度）**: 記事一覧、軽微な修正、カテゴリ/タグ管理
- **優先度低（PC前提のまま・投資しない）**: 記事執筆、サムネイルトリミング、セクション編集、バックアップ/リストア
- 進め方: ①スマホ幅スクリーンショットで実態調査（推測で設計しない） → ②機能仕様書作成 → 承認 → ③実装
- 実施時期: **S1-7の後**（P3のビュー整理との二度手間防止）。PWA化はステージ2の題材
- 補足: パスキーログイン（iPhone Face ID）は本番検証済みで、モバイル管理との相性は既に良い
...
## S1-4 実施記録（2026-07-16）

設定反映の信頼性4件を完了。全1,268specグリーン。

- **C-9**: SiteSettingのクラス変数メモ化を廃止し、共有キャッシュ（solid_cache）のみに依存。「別プロセスのキャッシュクリアが反映される」回帰テスト追加（旧実装で失敗することを確認済み）。デッドコード（fetch_setting/preload_common_settings/cache_stats等）も削除
- **M-2**: DBインポート完了後に `Rails.cache.clear` を実行（旧設定・旧記事一覧のキャッシュ残留を解消）。**発見**: test環境は `null_store` のためキャッシュ系テストが素通りになる → MemoryStore差し替えで実効性のあるテストに修正
- **H-7**: site_title/site_description を全ページの `<title>`・OGP・meta descriptionに反映
  - タイトル生成を `page_title` ヘルパーに一元化（サイト名の付与はレイアウトのみ＝**二重付与防止**。剛さん指摘の設計世代重複への対策）
  - 各ビューの `content_for(:title)` は「ページ固有部分のみ」に統一
  - MetaTagsServiceのハードコード（宮川 剛/Miyakawa Codes Blog）を設定値参照に置換、og:site_nameも設定反映
  - 死にコード発見: `content_for(:description)` はレイアウトのどこからも使われていなかった（削除）
  - 二重出力防止の回帰テスト付き（`spec/requests/site_title_reflection_spec.rb`）
  - ※ヘッダー/フッターの**ブランドテキスト**はsite_titleの意味（タブ・SNS用）を超えるためC-8（S2）で専用キーを設けて対応
- **C-10（案B承認済み）**: `AdminPathRouteReloader` ミドルウェア新設
  - リクエスト前に現在パスとルート構築時パスを照合（5秒間引き）、変更検知で自プロセスのルートを再読み込み
  - **dev実地検証済み**: 別プロセスからのDB履歴変更が再起動なしで反映（新パス302/旧パス404）、履歴削除で自動復帰も確認
...
## S1-6 Phase A 実施記録（2026-07-16）

パスキー認証を仕様駆動+TDDで実装（仕様書: `docs/specifications/features/passkey_authentication.md`、新体制での初サイクル）。全1,231specグリーン。

- **2FA是正**: OTP許容ドリフトを前後60秒に明示拡大（`Devise.otp_allowed_drift`。時計ズレによる「正しいコードが弾かれる」対策）、`:timeoutable` 廃止（剛さん決定）
- **パスキー基盤**: `webauthn` gem 3.4、`passkey_credentials` テーブル（公開鍵のみ保存）、`AdminUser#webauthn_id` 自動生成、sign_count逆行によるクローン検知
- **登録**: パスワード再確認付きチャレンジ発行→検証。ニックネーム（同一ユーザー内一意）・上限10・既存除外・メール通知
- **ログイン**: ログイン画面にパスキーボタン（discoverable credential・生体認証必須）。成功時はOTP不要。パスワード+2FAは並行運用（Phase A）
- **リカバリー**: 1系統のみ警告バナー（管理画面全体）、最終パスキーの削除ガード（2FA有効時のみ許可）、クローン検知時のログイン拒否+警告メール、rack_attackレート制限（10回/分）
- **検証**: FakeClientによるrequest spec 20例+モデル10例。**実機: MacBook Pro Touch IDで登録・ログイン成功**（剛さん確認）
- **本番受け入れ完了（2026-07-16）**: デプロイ成功。Mac登録→iPhone Face IDログイン（iCloud同期鍵）→Chrome（Googleプロファイル）2つ目登録、すべて成功。2系統リカバリー成立・警告バナー消灯
- **時計ズレ調査の結論**: コンテナ(UTC)とホスト(JST)は秒まで一致＝**ズレゼロ**（表示TZの違いのみ）。2FA不安定の原因は時計ではなく、rememberトークン連鎖/タイムアウトの体感（仮説1・2）だった可能性が高い。タイムアウト廃止+パスキー移行で実質解消見込み。経過観察へ
- 残: Phase B（パスワードレス化+ローテーション機構廃止）は安定稼働の確認後
