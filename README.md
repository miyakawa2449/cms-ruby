...
6. **AI支援機能**: Amazon Bedrock連携（タイトル・要約・タグ・スラッグ・SEOメタ提案）
7. **コンタクトフォーム**: AWS SES連携・スパム対策
8. **管理画面**: Devise認証・権限管理
9. **データベース管理**: エクスポート/インポート機能（ZIP形式）
...
### 運用メモ
- AI集計グラフは **インラインJSではなく Stimulus で描画**（CSP対策）
...
管理画面のダッシュボードから、データベースとActive Storageファイルをエクスポート/インポートできます。

### エクスポート

1. 管理画面ダッシュボードの「エクスポート」ボタンをクリック
2. ZIPファイルが自動的にダウンロードされます

**ZIPファイルの内容:**
- `data.json`: 全モデルのデータ（AdminUser、Article、Category、Tag等）
- `storage/`: Active Storageファイル（画像等）

### インポート

1. 管理画面ダッシュボードの「インポート」ボタンをクリック
...
エクスポート/インポートの対象となるモデル:
- AdminUser、Article、Category、Tag
- Section、SectionContent、MyStorySection
- ArticleCategory、ArticleTag（中間テーブル）
- Contact、SiteSetting
